...
echo [build] stopping running Controledu-related processes
...
call dotnet build Controledu.sln -c Release || exit /b 1

call :publish_release_outputs || exit /b 1
...
echo [build] publish Teacher.Host
...
echo [build] publish Student.Host
...
if exist "%ROOT%\favicon.png" (
  copy /y "%ROOT%\favicon.png" "%PUBLISH_ROOT%\teacher-host\wwwroot\favicon.png" >nul || exit /b 1
  copy /y "%ROOT%\favicon.png" "%PUBLISH_ROOT%\student-host\wwwroot\favicon.png" >nul || exit /b 1
)
if exist "%ROOT%\favicon.svg" (
  copy /y "%ROOT%\favicon.svg" "%PUBLISH_ROOT%\teacher-host\wwwroot\favicon.svg" >nul || exit /b 1
  copy /y "%ROOT%\favicon.svg" "%PUBLISH_ROOT%\student-host\wwwroot\favicon.svg" >nul || exit /b 1
)

echo [build] publish Student.Agent
...
echo [build] publish Controledu.Updater
...
if "%ISCC_PATH%"=="" (
  echo [build] ERROR: Inno Setup ISCC.exe not found.
  echo [build] Set INNO_ISCC environment variable to full path of ISCC.exe
  exit /b 1
)

if not exist "%ISCC_PATH%" (
  echo [build] ERROR: ISCC.exe does not exist at "%ISCC_PATH%"
  exit /b 1
)

echo [build] building Inno installers (version %APP_VERSION%)
...
:sync_assets
echo [build] syncing shared favicon assets
if not exist "%ROOT%\favicon.ico" (
  echo [build] missing %ROOT%\favicon.ico
  exit /b 1
)
...
if exist "%ROOT%\favicon.png" (
  ...
  echo [build] warning: favicon.png not found, skipping png sync
)
